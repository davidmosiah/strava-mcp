...
import type { StravaTokenSet } from "../types.js";
...
export class TokenStore {
  ...
  async read(): Promise<StravaTokenSet | null> {
    try {
      const text = await fs.readFile(this.tokenPath, "utf8");
      return JSON.parse(text) as StravaTokenSet;
    } catch (error) {
  ...
  async write(tokens: StravaTokenSet): Promise<void> {
