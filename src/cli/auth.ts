...
export async function runAuthCommand(args: string[]): Promise<number> {
  ...
  const result = await waitForOAuthCode(redirect, state, timeoutMs, async (url) => {
    if (!json) {
      console.log("Strava MCP · Authorization");
      console.log("");
      if (noOpen) {
      ...
      console.log("  2. Strava will redirect to the local callback.");
  ...
  const output = {
    ...
    next_step: "Run `strava-mcp-server doctor`, then add the MCP server to your agent."
  };
...
function waitForOAuthCode(
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      server.close();
      reject(new Error("Timed out waiting for STRAVA OAuth callback."));
    }, timeoutMs);
    ...
    const server = createServer((req, res) => {
      try {
        ...
        if (!code) throw new Error("STRAVA callback did not include a code.");
        if (state !== expectedState) throw new Error("STRAVA callback state mismatch.");
...
<head>
  ...
  <title>Strava connected · Delx Wellness</title>
  <style>
...
<body>
  ...
  <ol>
    ...
    <li>Run <code>strava-mcp-server doctor</code> to verify scopes and setup.</li>
