...
function writeHermesClientConfig(homeDir: string): ClientConfigResult {
  ...
  return {
    ...
    warnings: [
      "After editing Hermes MCP config, use `/reload-mcp` or `hermes mcp test strava`; do not restart the Hermes gateway for normal Strava data access.",
...
function mergeHermesConfig(configPath: string): string | undefined {
  ...
  if (/strava-mcp-unofficial|strava-mcp-server|strava-mcp/.test(existing) && /^\s*strava\s*:/m.test(existing)) {
    if (existing.includes(PINNED_NPM_PACKAGE)) return undefined;
    const backupPath = backupConfig(configPath);
    const updated = existing.replace(/strava-mcp-unofficial(?:@\d+\.\d+\.\d+)?/g, PINNED_NPM_PACKAGE);
  ...
  const backupPath = backupConfig(configPath);
  const next = existing.trimEnd().length ? addHermesStravaBlock(existing) : snippet;
  writeFileSync(configPath, ensureReloadHint(next), { mode: 0o600 });
  chmodSync(configPath, 0o600);
  return backupPath;
}

function addHermesStravaBlock(existing: string): string {
  const serverBlock = [
    "  strava:",
    "    command: npx",
    "    args:",
...
function backupConfig(path: string): string {
  const backupPath = `${path}.bak-strava-mcp-${new Date().toISOString().replace(/[-:]/g, "").replace(/\..+/, "Z")}`;
...
function setupNextStep(client: AgentClientName, noAuth: boolean): string {
  const auth = noAuth ? "Run `strava-mcp-server auth`, then " : "";
  if (client === "hermes") {
    return `${auth}run \`strava-mcp-server doctor --client hermes\`, then use \`/reload-mcp\` or \`hermes mcp test strava\`.`;
  }
  return `${auth}run \`strava-mcp-server doctor\`.`;
}
