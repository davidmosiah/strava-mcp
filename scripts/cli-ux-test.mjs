...
const dir = mkdtempSync(join(tmpdir(), 'strava-mcp-cli-'));

try {
  const missing = await buildConnectionStatus({ env: {}, homeDir: dir, nowMs: 1_000_000 });
  assert.equal(missing.ok, false);
  assert.equal(missing.ready_for_strava_api, false);
  assert.deepEqual(missing.missing_env, ['STRAVA_CLIENT_ID', 'STRAVA_CLIENT_SECRET', 'STRAVA_REDIRECT_URI']);
  assert.ok(missing.next_steps.some((step) => step.includes('STRAVA_CLIENT_ID')));
  ...
  assert.ok(doctorPayload.next_steps.some((step) => step.includes('STRAVA_CLIENT_ID')));
  ...
  assert.match(authWithoutEnv.stderr, /Missing required STRAVA environment variables/);
  ...
  const configPath = join(dir, '.strava-mcp', 'config.json');
