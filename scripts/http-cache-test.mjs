...
process.env.STRAVA_NO_CACHE = 'true';
{
  let calls = 0;
  const inner = async () => { calls += 1; return okResponse(); };
  const opts = { innerFetch: inner, defaultTtlSeconds: 10, envVarBypass: 'STRAVA_NO_CACHE' };
  await fetchWithCache('https://api.example/v1/bypass', { method: 'GET' }, opts);
  await fetchWithCache('https://api.example/v1/bypass', { method: 'GET' }, opts);
  assert.equal(calls, 2, 'STRAVA_NO_CACHE=true must skip cache reads AND writes');
  assert.equal(getCacheStats().size, 0);
}
delete process.env.STRAVA_NO_CACHE;
