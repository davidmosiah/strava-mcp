...
const pinnedPackage = `strava-mcp-unofficial@${packageJson.version}`;
const dir = mkdtempSync(join(tmpdir(), 'strava-mcp-hermes-agent-'));

const client = new Client({ name: 'strava-mcp-hermes-agent-test', version: '0.0.0' });
...
try {
  ...
  assert.ok(toolNames.includes('strava_agent_manifest'), 'Hermes-ready agent manifest tool should be registered.');

  const resources = await client.listResources();
  const resourceUris = resources.resources.map((resource) => resource.uri);
  assert.ok(resourceUris.includes('strava://agent-manifest'), 'Agent manifest resource should be registered.');

  const manifestResult = await client.callTool({
    name: 'strava_agent_manifest',
  ...
  assert.equal(manifest.hermes.tool_name_prefix, 'mcp_strava_');
  assert.equal(manifest.hermes.no_gateway_restart_for_data_access, true);
  assert.equal(manifest.hermes.reload_after_config_change, '/reload-mcp');
  assert.ok(manifest.hermes.common_tool_names.includes('mcp_strava_strava_connection_status'));
  ...
  const manifestResource = await client.readResource({ uri: 'strava://agent-manifest' });
  const text = manifestResource.contents[0]?.text ?? '';
  assert.match(text, /mcp_strava_strava_connection_status/);
  ...
  assert.ok(setupPayload.hermes_skill_path.endsWith('.hermes/skills/strava-mcp/SKILL.md'));
  ...
  assert.match(readFileSync(setupPayload.hermes_skill_path, 'utf8'), /mcp_strava_strava_connection_status/);
  ...
  assert.equal(doctorPayload.client_checks.hermes.strava_server_configured, true);
  ...
  const mergeDir = mkdtempSync(join(tmpdir(), 'strava-mcp-hermes-merge-'));
  ...
  assert.match(mergedConfig, /strava:/);
